...
void main()
{
	...
	*/
	...
	gl_FragColor = vec4(texture2D(skytex,vec2((theta/3.141592653582+1.0)/2.0,1.0-phi/3.14159265358)).rgb,1.0);
	
	//vec3 map = ((diff/sqrt(dot(diff,diff)))+vec3(1.0))*0.5;
	//gl_FragColor = vec4(texture3D(skytex,vec2((atan(map.z/map.x)/3.14159265358+1.0)/2.0,map.y)).rgb,1.0);
}